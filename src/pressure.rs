...
impl PressureSettings {
    ...
    /// - DEC[0:1] - Decimation of pressure data on OUT REG and FIFO.
    ///     - 00: None
    ///     - 01: 2 samples
    ///     - 10: 4 samples
    ///     - 11: 8 samples
...
/// Pressureerometer full-scale selection. (Refer to Table 67)
...
impl Scale {
    ...
    /// Returns Linear pressureeration sensitivity depending on scale. (Refer to Page 12)
...
/// Pressureerometer digital filter (high pass and low pass) cutoff frequency selection:
...
fn pressure_scale_values() {
    ...
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0000_0000);

    let pressure = PressureSettings {
        scale: _4G,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0001_0000);

    let pressure = PressureSettings {
        scale: _8G,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0001_1000);

    let pressure = PressureSettings {
        scale: _16G,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0000_1000);
}
...
fn pressure_set_odr() {
    ...
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0010_0000);

    let pressure = PressureSettings {
        sample_rate: _50Hz,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0100_0000);

    let pressure = PressureSettings {
        sample_rate: _119Hz,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0110_0000);

    let pressure = PressureSettings {
        sample_rate: _238Hz,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b1000_0000);

    let pressure = PressureSettings {
        sample_rate: _476Hz,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b1010_0000);

    let pressure = PressureSettings {
        sample_rate: _952Hz,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b1100_0000);
}

#[test]
fn set_pressure_bandwidth_selection() {
    ...
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0000_0100);
...
fn set_pressure_bandwidth() {
    ...
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0000_0001);

    let pressure = PressureSettings {
        bandwidth: _105Hz,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0000_0010);

    let pressure = PressureSettings {
        bandwidth: _50Hz,
        ..Default::default()
    };
    assert_eq!(pressure.ctrl_reg6_xl() & mask, 0b0000_0011);
